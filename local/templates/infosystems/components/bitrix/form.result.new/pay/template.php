...
<form action="<?=$arParams['FORM_ACTION']?>" class="js-validate forms js-form form-pay" role="form" method="post">
    ...
    <div class="flex-row">
        ...
        <?php if ($arParams['LINK_AGREEMENT']):?>
            <div class="row row-checkbox">
                <?php
        ...
        <?php else:?>
            <p class='privacy-text'>
                Оформляя заказ, вы соглашаетесь с <a href="<?=!empty($arParams['LINK_AGREEMENT']) ? $arParams['LINK_AGREEMENT'] : '/about/privacy/'?>" target="_black">обработкой персональных данных</a>
            </p>
        <?php endif?>
