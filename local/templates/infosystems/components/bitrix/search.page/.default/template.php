...
<?$this->SetViewTarget("search-page-form");?>
	<div class="search-panel in-search">
		<div class="container">
			<form action="" method="get" class="row search-form">
				<div class="col-12 col-sm-12 col-md-7 col-lg-7 col-xl-7">
					<?if($arParams["USE_SUGGEST"] === "Y"):
						...
						<?$APPLICATION->IncludeComponent("bitrix:search.suggest.input","",
							array(
								"NAME" => "q",
					...
					<?else:?>
						<input type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" class="search-input" />
					<?endif;?>
				</div>
				<?if($arParams["SHOW_WHERE"]):?>
					<div class="col-10 col-sm-10 col-md-4 col-lg-3 col-xl-4">
						<select style="display:none" name="where" id="search-category-in" class="search-category">
							<option value=""><?=GetMessage("SEARCH_ALL")?></option>
							<?foreach($arResult["DROPDOWN"] as $key=>$value):?>
								<option value="<?=$key?>"<?if($arResult["REQUEST"]["WHERE"]==$key) echo " selected"?>><?=$value?></option>
							<?endforeach?>
						</select>
						<div class="nice-select search-category-in search-category">
							<span class="current"><?=GetMessage("SEARCH_ALL")?></span>
							<ul class="list">
								<li data-value class="option"><?=GetMessage("SEARCH_ALL")?></li>
								<?foreach($arResult["DROPDOWN"] as $key=>$value):?>
									<li data-value="<?=$key?>" class="option<?=$arResult["REQUEST"]["WHERE"]==$key?' selected':''?>"><?=$value?></li>
								<?endforeach?>
							</ul>
						</div>
					</div>
				<?endif;?>
				<div class="col-2 col-sm-2 col-md-1 col-lg-2 col-xl-1 search-button">
					<span class="input-group-btn">
						<button type="submit" class="button button--round button--secondary icon-search" /><span class="glyphicon glyphicon-search"></span> <?=GetMessage("SEARCH_GO")?></button>
					</span>
				</div>
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
		...
		<p><?echo strip_tags(html_entity_decode($arItem["BODY_FORMATED"]),'<b>')?></p>
		<?if (
