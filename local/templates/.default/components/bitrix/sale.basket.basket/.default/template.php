...
if (empty($arResult['ERROR_MESSAGE']))
{
    ...
    /*foreach ($arResult['BASKET_ITEM_RENDER_DATA'] as &$DATUM){
	    if(!empty($DATUM['COLUMN_LIST'])){
	        foreach ($DATUM['COLUMN_LIST'] as $k=>$item){
                if(!in_array($item['CODE'],['PROPERTY_DATE_VALUE','PROPERTY_CITY_VALUE'])) continue;
                //
                $item['VALUE'] = ($item['CODE']=='PROPERTY_DATE_VALUE'?FormatDate("d F Y",strtotime($item['VALUE'])):$item['VALUE']);
                $DATUM[$item['CODE']] = $item['VALUE'];
                unset($DATUM['COLUMN_LIST'][$k]);
            }
        }
    }*/
    //
	...
	<script>
		...
		BX.Sale.BasketComponent.init({
			...
			course_dates_list: <?=CUtil::PhpToJSObject($arResult["COURSE_DATES_LIST"])?>,
