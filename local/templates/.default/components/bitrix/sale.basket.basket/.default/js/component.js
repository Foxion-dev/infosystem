;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		init: function(parameters)
		{
			...
			this.course_dates_list = parameters.course_dates_list;
		...
		checkOutAction: function()
		{
			...
			$(".basket-product-date").each(function(){
			...
			if (result.BASKET_ITEM_RENDER_DATA)
			{
				...
				for (i in result.BASKET_ITEM_RENDER_DATA)
				{
					if (result.BASKET_ITEM_RENDER_DATA.hasOwnProperty(i))
					{
						...
						if(typeof this.course_dates_list[result.BASKET_ITEM_RENDER_DATA[i].PRODUCT_ID] != "undefined"){
							result.BASKET_ITEM_RENDER_DATA[i].COURSE_DATES_LIST = [];
							result.BASKET_ITEM_RENDER_DATA[i].COURSE_DATES_LIST = this.course_dates_list[result.BASKET_ITEM_RENDER_DATA[i].PRODUCT_ID];
						}
		...
		renderBasketItem: function(template, data)
		{
            ...
            //console.log(clonedData['COLUMN_LIST']);
			var arr = clonedData['COLUMN_LIST'];
            arr.forEach(function(item, i, arr) {
                //alert( i + ": " + item + " (массив:" + arr + ")" );
				if(item['CODE']=='PROPERTY_DATE_VALUE' || item['CODE']=='PROPERTY_CITY_VALUE'){
                    if(item['CODE']=='PROPERTY_DATE_VALUE'){
                    	var ms = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"];
                        var dt = item['VALUE'].split(' ');
                        var dtl = dt[0].split('.');
                        /*console.log(dtl);
                        console.log(ms[dtl[1]-1]);*/
                        item['VALUE'] = dtl[0] + ' ' +ms[dtl[1]-1]+' '+ dtl[2];
					}
                    clonedData[item['CODE']] =  item['VALUE'];
                    //
                    /* console.log(i);
                    console.log(item);
                    console.log(clonedData['COLUMN_LIST'][i]);
                    delete clonedData['COLUMN_LIST'][i];*/
                }
            });
