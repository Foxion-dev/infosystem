	...
	<td>
		...
		?>
			...
			<?if (array_key_exists($index+1, $arResult["arMap"]) && $arItem["LEVEL"] < $arResult["arMap"][$index+1]["LEVEL"]):?>
				...
				<li  class="LI_LEVEL_<?=$arItem["LEVEL"]?>"><a href="<?=$arItem["FULL_PATH"]?>" class="LEVEL_<?=$arItem["LEVEL"]?>"><?=$arItem["NAME"]?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><div><?=$arItem["DESCRIPTION"]?></div><?}?>
					<ul class="map-level-<?=$arItem["LEVEL"]+1?>">

			<?else:?>

					<li class="LI_LEVEL_<?=$arItem["LEVEL"]?>"><a href="<?=$arItem["FULL_PATH"]?>" class="LEVEL_<?=$arItem["LEVEL"]?>"><?=$arItem["NAME"]?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><div><?=$arItem["DESCRIPTION"]?></div><?}?></li>
