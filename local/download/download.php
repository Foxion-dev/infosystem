...
$DIR = dirname($sCurUrl);
//AddMessage2Log("\n".var_export($DIR, true). " \n \r\n ", "DIR");
//AddMessage2Log("\n".var_export($sCurUrl, true). " \n \r\n ", "DIR");
$file = $rel = substr($sCurUrl, strlen($DIR)+1);
$GA = CFile::GetFileArray($file);
$file = $GA['FILE_NAME'];
$filename = $_SERVER["DOCUMENT_ROOT"].$GA['SRC'];
//

if(file_exists($filename) && is_file($filename) && $GA)
{
	//require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_before.php");
	...
	// проверка на алгоритм приобретения этого файла ищем по покупкам и т.д.
	if(!$USER->IsAuthorized())
		LocalRedirect($DIR."/auth.php?DIR=".urlencode($sCurUrl)."&rel=".$rel);
	else
	{
