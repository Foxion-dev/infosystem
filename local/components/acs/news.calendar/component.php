...
if($arParams["TYPE"] || !in_array($arParams["DATE_FIELD"], array("DATE_ACTIVE_FROM", "DATE_ACTIVE_TO", "TIMESTAMP_X", "DATE_CREATE", "PROPERTY_DATE")))
	$arParams["DATE_FIELD"] = "PROPERTY_DATE";
...
//
if($this->StartResultCache(false,[$currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay, $USER_ID]))
{
	...
	//
    $arResult["ID_EVENT"] = $this->getArrEvent($USER_ID);
	...
	// если есть ID_EVENT то по ним фильруем и т.д.
    // p($arResult["ID_EVENT"],'p');
	if(count($arResult["ID_EVENT"])) {
        $arFilter = Array(
            "ACTIVE" => "Y",
        ...
        while ($arItem = $dbItems->GetNext()) {
            //  p($arItem,'p');
            $arDays[ConvertDateTime($arItem['PROPERTY_DATE_VALUE'], CLang::GetDateFormat("SHORT"))][] = $arItem;
        }
    }
	//
	...
	if($arParams["SHOW_YEAR"])
	{
		$arFilter = Array(
			"ACTIVE" => "Y",
			"<".$arParams["DATE_FIELD"] => date("Y-m-d H:i:s", mktime(0,0,0,1,1,$currentYear)),
			"IBLOCK_ID" => $arParams["IBLOCK_ID"],
		);
	...
	if($bCheckNext)
	{
		if($arParams["SHOW_YEAR"])
		{
			$arFilter = Array(
				"ACTIVE" => "Y",
				">=".$arParams["DATE_FIELD"] => date("Y-m-d H:i:s", mktime(0,0,0,1,1,$currentYear+1)),
				"IBLOCK_ID" => $arParams["IBLOCK_ID"],
			);
			$dbItems = CIBlockElement::GetList(array(), $arFilter, false, array("nTopCount"=>1), Array("ID", "IBLOCK_ID"));
			if($arItem = $dbItems->GetNext())
		...
		for ($j = 0; $j < 7; $j++)
		{
			...
			if(is_set($arDays[$tmpDate]))
			{
				...
				foreach($arDays[$tmpDate] as $dayNews)
				{
					...
					{
						$arTime = ParseDateTime($dayNews["PROPERTY_DATE_VALUE"], CLang::GetDateFormat("FULL"));
