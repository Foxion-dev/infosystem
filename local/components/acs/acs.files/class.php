...
class acsFilesClass extends \CBitrixComponent
{
    ...
    public function UserAddPhoto($get){
        $arJson = [];
        if (strlen($get["NAME"]) == 0) {
            $error['NAME'] = "Обязательное поле: Название для галерей";
        }
        //
        if(count($_FILES['file'])) { // its ok
        }else{
            $error['file'] = "Пожалуйста добавьте файлы";
        }
        //AddMessage2Log("\n".var_export($get, true). " \n \r\n ", "get");
        ...
        if (is_array($get) && count($error) == 0) {
            ...
            if(count($_FILES['file'])):
                //
                foreach ($_FILES['file']['error'] as $f=>$er):
                    if($er != 0 ) continue;
                    $tmpFile = Array(
            ...
            $arLoadProductArray = Array(
                ...
                'IBLOCK_SECTION_ID' => $get['IBLOCK_SECTION_ID'], // элемент лежит в корне раздела
                'IBLOCK_ID' => 6,
            ...
            //
            $html .= ' Спасибо, ваша галерея успешно отправлена и будет активированна в рабочее время пн-пт с 09 до 18.';
    ...
    public function executeComponent()
    {
        ...
        $rsSect = CIBlockSection::GetList(['sort'=>'asc'],$arFilter,false,['ID','NAME','CODE']);
