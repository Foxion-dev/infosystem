...
<div class="catalog-section-courses">
    <?foreach($arResult["ITEMS"] as $c=>$arElement):?>
        ...
        ?>
        <? if($c==3 && intval($_REQUEST['PAGEN_1'])==0):?><div class="heading-wrapper heading-wrapper-oll"><h5><?='Все курсы';?></h5></div><?endif;?>
        <div class="nearest-courses-items" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
            <div class="row">
                <div class="col-12"><div class="nearest-courses-items-title"><div class="date icon-calendar">
                    <?=(!empty($arElement['DISPLAY_PROPERTIES']['DATE']['VALUE'])?FormatDate("d F Y",strtotime($arElement['DISPLAY_PROPERTIES']['DATE']['VALUE'])):'Уточняйте у менеджера')?></div>   |   <?=$arElement['DISPLAY_PROPERTIES']['CITY']['VALUE']?>   |   Код - <?=$arElement['PROPERTIES']['ARTNUMBER']['VALUE']?></div></div>
