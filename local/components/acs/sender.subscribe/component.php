...
// основной алгоритм подписки на рассылку и т.д.
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid() && isset($_POST['sender_subscription']) && $_POST['sender_subscription']=='add' && $_POST['go']=='subscribe')
{
    //
    if (strlen($_POST["sendCopy"]) == 0) {
        $error['sendCopy'] = "Обязательное поле: Согласен на обработку персональных данных";
        $arJson = [];
        $APPLICATION->RestartBuffer();
        $arJson['jq']['html'] = array("#alert-subscribe-danger-error" => '<small>'.$error['sendCopy'].'</small>');
    ...
    if(check_email($_POST["SENDER_SUBSCRIBE_EMAIL"], true))
	{
		...
		//
        // чистим буфер и выводим сообщение
        $arJson = [];
        $APPLICATION->RestartBuffer();
        ob_start();
        print $messageDictionary[$arResult['MESSAGE']['CODE']]['TEXT'];
        ...
        $arJson['jq']['hide'][".bx_subscribe_subform"] = 300; // просто схлопываем форму и т.д.
        ...
        $arJson = [];
        $APPLICATION->RestartBuffer();
        $arJson['jq']['html'] = array("#alert-subscribe-danger-error" => '<small>'.$messageDictionary[$arResult['MESSAGE']['CODE']]['TEXT'].'</small>');
        $arJson['jq']['show'] = array("#alert-subscribe-danger-error" => 300);
        $arJson['jq']['hide'] = array("#alert-subscribe-danger-success" => 300);
        //
        print json_encode($arJson);
