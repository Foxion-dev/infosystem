	...
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if($isOrderPlaced || $isPersonTypeChanged)
		{
          ...
          if (!$APPLICATION->CaptchaCheckCode($_POST['captcha'], $_POST['captcha_sid'])) {
            $arResult["ERROR"][] = "Укажите верный код с картинки: ".$_POST["captcha"]." ? ";
          }
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				// account replenishment, etc. -- пополнение счета и т.д.
                if (empty($arResult["ERROR"])){
                    addUserBonus($arFields["USER_ID"],$arResult["BASKET_ITEMS"],$arResult["ORDER_ID"]); // добавляет бонусы и т.д.
                    /* AddMessage2Log("\n" . var_export($arFields["USER_ID"], true) . " \n \r\n ", "USER_ID");
                    AddMessage2Log("\n" . var_export($arResult["BASKET_ITEMS"], true) . " \n \r\n ", "BASKET_ITEMS"); */
                }
                // end account replenishment, etc.
