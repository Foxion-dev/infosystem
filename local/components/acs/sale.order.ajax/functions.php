...
function addUserBonus($USER_ID, $BASKET_ITEMS, $ORDER_ID = Null){
    if(!empty($BASKET_ITEMS) && intval($USER_ID)>0 && CModule::IncludeModule('iblock') && CModule::IncludeModule("sale") && CModule::includeModule('highloadblock')){
        $AID = [];
        ...
        while($item = $res->GetNext()){
            if(!$item['PROPERTY_BONUS_VALUE']) continue;
            $BONUS = $BONUS + intval($item['PROPERTY_BONUS_VALUE']);
        }
        // если больше 0 то начисляем и т.д.
        if(intval($BONUS)>0){
            if($arr = CSaleUserAccount::GetByUserID($USER_ID, "RUB")){
                // если он есть
                $BID = $arr['ID'];
                $arr["CURRENT_BUDGET"] = $BONUS + intval($arr["CURRENT_BUDGET"]);
                unset($arr['ID']);
                CSaleUserAccount::Update($BID,$arr);
            }else{
                // если нету
                $arFields = ["USER_ID"=>$USER_ID, "CURRENCY"=>"RUB", "CURRENT_BUDGET"=>$BONUS];
                CSaleUserAccount::Add($arFields);
            }
            // пишем историю для бонусов в HL блок и т.д.
            if($ORDER_ID && CModule::IncludeModule('acs')){
                // запись и т.д. UF_CODE  credit && debet
                $dataFields = [
                    "UF_USER" => $USER_ID,
                    "UF_ORDER" => $ORDER_ID,
                    "UF_BONUS" => $BONUS,
                    "UF_DATE" => date('d.m.Y H:i',time()),
                    "UF_CODE" => 'CREDIT', // кредитуем пользователя
                ];
                HiWrapper::id(7)->add($dataFields);
            }
        }
    }
}